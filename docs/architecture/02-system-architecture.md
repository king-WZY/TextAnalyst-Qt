...
| 22.04 LTS | Jammy | 5.15  | 2.35 | 5.15.13 / 6.2.4 | 11.4 | **维护态** | v1.2 修订：apt 实际 Qt6 为 6.2.4（6.4.2 仅 24.04 有），不满足 §1.2 的 Qt6.4 要求，CI 基线废止；且 runner 镜像 2026-09-17 起弃用 |
| 24.04 LTS | Noble | 6.8   | 2.39 | 6.4.2 | 13.3 | **稳定支持（开发主环境）** | CI 唯一基线（v1.2 修订） |

- **MUST**（v1.2 修订）：CI 仅在 `ubuntu-24.04` runner 上跑通全部
  单元/集成/打包测试。原"22.04+24.04 双基线"废止——22.04 的 apt Qt6
  为 6.2.4，不满足 §1.2 的 Qt6.4 下限，双矩阵在 22.04 上本就不可构建；
  ubuntu-22.04 runner 镜像亦于 2026-09-17 起弃用。
